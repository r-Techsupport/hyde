...
async fn get_oath_processor(
    ...
    if response.status().is_success() {
        ...
        if !all_users.iter().any(|u| u.username == username) {
            let expiration_date = Utc::now() + token_data.expires_in().wrap_err("Discord OAuth2 response didn't include an expiration date")?;
            db::create_user(&state.db_connection_pool, username.to_string(), token.to_string(), expiration_date.to_rfc3339()).await?;
